...
// ----------------------------------------------------------------------
/*!
 * Muuttaa parametrina annetun gridin halutessa tihe‰mp‰‰n/v‰ljemp‰‰n
 * hilaan lineaarisesti interpoloiden, alueiden oletetaan olevan samoja.
 *
 * \param theData L‰htˆdata, josta initialisoidaan uusi hila (=this olio).
...
bool NFmiGridBase::Init(NFmiGridBase *theData)
{
  ...
  {
    // HUOM! - ei ota huomioon "croppausta", ts. toimii aina KOKO gridille, ei pelk‰st‰‰n sen
    // osagridille.
...
{
  // HUOM!!! En tied‰ varmana, miten croppaus pit‰isi ottaa huomioon t‰ss‰ laskussa!!! Marko
...
std::ostream &NFmiGridBase::Write(std::ostream &file) const
{
  ...
  {
    NFmiDataPool pool;  // Salla teki pidemm‰n kaavan kautta v‰ltt‰‰kseen varoituksen..
    file << pool;
  }
...
// ----------------------------------------------------------------------
/*!
 * Tein t‰m‰n Swap-funktion , alkuper‰inen on outo siin‰ mieless‰, ett‰
 * sill‰ on joku startincorner olemassa. Jos se ei ole bottomleft, tehd‰‰n
 * swappi‰, mutta startingcorner j‰‰ kuitenkin sellaiseksi kuin se oli.
 * Nyt haluan kertoa gridille, ett‰ koska sin‰ olet v‰‰rin p‰in,
 * muuta datasi seuraavasti, mutta lopuksi startingcorner pit‰‰ olla
 * bottomleft, mihin muotoon lopuksi data j‰‰.
 *
...
bool NFmiGridBase::Swap(FmiDirection theCurrentDirection)
{
  FmiDirection oldDirection =
      itsStartingCorner;  // itsStartingCorner pit‰isi pit‰‰ sis‰ll‰‰n bottomleft:in
...
// gridbasen yht‰l‰isyydest‰ ei oikeastaan muuta dataosiot kiinnosta kuin hilan x- ja y-koot.
